...
subplot(2,1,2)
lambdamatrix=linspace(300,700,20);
m=1;          % Diffraction Order 
anglematrix1 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle1)==0;x1=-cos(r-angle1);y1=sin(r-angle1);else x1=0;y1=0;end;
m=-1;          % Diffraction Order 
anglematrix2 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle2)==0;x2=-cos(r-angle2);y2=sin(r-angle2);else x2=0;y2=0;end;
m=2;          % Diffraction Order 
anglematrix3 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle3)==0;x3=-.8.*cos(r-angle3);y3=.8.*sin(r-angle3);else x3=0;y3=0;end;
m=-2;          % Diffraction Order 
anglematrix4 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle4)==0;x4=-.8.*cos(r-angle4);y4=.8.*sin(r-angle4);else x4=0;y4=0;end;
m=3;          % Diffraction Order 
anglematrix5 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle5)==0;x5=-.6.*cos(r-angle5);y5=.6.*sin(r-angle5);else x5=0;y5=0;end;
m=-3;          % Diffraction Order 
anglematrix6 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle6)==0;x6=-.6.*cos(r-angle6);y6=.6.*sin(r-angle6);else x6=0;y6=0;end;
m=4;          % Diffraction Order 
anglematrix7 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle7)==0;x7=-.4.*cos(r-angle7);y7=.4.*sin(r-angle7);else x7=0;y7=0;end;
m=-4;          % Diffraction Order 
anglematrix8 = asin(m*lambdamatrix./d-sin(alphar));if imag(angle8)==0;x8=-.4.*cos(r-angle8);y8=.4.*sin(r-angle8);else x8=0;y8=0;end;
anglematrix= cat(1, anglematrix1,anglematrix2,anglematrix3,anglematrix4,anglematrix5,anglematrix6,anglematrix7,anglematrix8);
anglematrix=abs(anglematrix);
pcolor(anglematrix)
% Draw the sliders
rtslid(h,@GratingMono1,h2,1,'Scale',[0 pi/2],'Def',alphar,'Back',[0.9 0.9 0.9],'Label','Angle','Position',[0.03 0.5 0.03 0.35]);
rtslid(h,@GratingMono2,h2,0,'Scale',[0 1000],'Def',500,'Back',[0.9 0.9 0.9],'Label','Wavelen.','Position',[0.95 0.05 0.03 0.35]);
